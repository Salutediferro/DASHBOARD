...
// Mifflin-St Jeor BMR + activity multiplier + goal offset. Pure compute,
// no DB reads — kept as an API endpoint so the macros logic lives in
// one place (used by both the coach plan builder and the patient
// onboarding "suggested macros" widget).
export async function POST(req: Request) {
  ...
  const goalOffset = goal === "CUTTING" ? -400 : goal === "BULKING" ? 300 : 0;
  const targetCalories = tdee + goalOffset;

  // Macro split: protein 2g/kg, fats 25% of kcal, rest carbs.
