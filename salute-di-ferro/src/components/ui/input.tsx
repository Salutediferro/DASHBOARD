...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  return (
    <InputPrimitive
      ...
      data-slot="input"
      className={cn(
        // Mobile-first height bump: h-10 on phones (~40px tap area) shrinking
        // to h-8 on desktop to preserve density. text-base on mobile is kept
        // to prevent iOS Safari's auto-zoom on focus (triggers when < 16px).
