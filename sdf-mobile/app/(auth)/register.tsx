...
export default function RegisterScreen() {
  ...
  async function onSubmit() {
    ...
    else Alert.alert("OK", "Controlla la tua email per confermare.");
  }

  return (
    <SafeAreaView className="flex-1 bg-bg-0">
      <View className="flex-1 justify-center px-6">
        <Text className="text-gold text-4xl font-bold mb-10" style={{ fontFamily: "Inter_700Bold" }}>
          Crea account
        </Text>
        ...
        <Link href="/(auth)/login" className="mt-6 text-center text-white/60">
          Hai già un account? <Text className="text-gold">Accedi</Text>
        </Link>
      </View>
    </SafeAreaView>
  );
